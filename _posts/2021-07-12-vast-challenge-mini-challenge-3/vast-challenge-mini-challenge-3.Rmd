...
![](img/4. Bi-Gram Time Series.jpg) {width=60%}
...
![](img/8. Retweet Network.jpg) {width=80%}
...
The period between 7.30 to 8.30pm seem to be the most dangerous periods, with phrases such as 'shots fired', 'gun fire' and 'shooters continue' appearing. It is only until the 9-9.30pm period where we see phrases like 'suspects arrested' and 'hostages saved' then can we reasonably conclude that danger has more or less subsided,

However, it is interesting to note as well the occurrence of a loud explosion being heard at possibly the Dancing Dolphin between the 9.30-10pm period. This might once again pose danger to civilians who are nearby.
...
As can be seen in the boxplot of tf-idf scores above, we will remove words with tf-idf score of 2.1 or higher, as these words most likely do not hold any insightful meaning for us. Subsequently, We will proceed to create the document term matrix required for LDA.
...
* Topics 3 and 5 seem to relate more to the POK Rally event
* Topics 4 and 6 seem to relate more to the fire outbreak at Dancing Dolphin
* Topics 1 seem to relate more to the hostage situation and shootout/standoff
* Topic 2 does not seem to have a distinctive topic attached to it (although it contains slight mentions of the rally and the hostage situation at Gelato Galore)


### Topics Across Time

We shall now plot these topics over time to see if there are certain periods throughout the night with increased happenings.

Take note that we have previously established that the word 'abila' appears all over the place throughout the night, and hence will likely not be meaningful in our analysis. Hence we shall remove the word 'abila' from Topic 4's top terms.


``` {r echo=TRUE}

fire_terms <- mb_words2_by_msg_top_terms %>% 
  filter(topic == 4 | topic == 6) %>% 
  filter(term != 'abila')
fire_terms_list <- fire_terms$term

hostage_terms <- mb_words2_by_msg_top_terms %>% 
  filter(topic == 1)
hostage_terms_list <- hostage_terms$term

rally_terms <- mb_words2_by_msg_top_terms %>% 
  filter(topic == 3 | topic == 5)
rally_terms_list <- rally_terms$term
...
fire_trend <- mb_words2 %>% 
  filter(word %in% fire_terms_list) %>% 
  mutate(sentiment = get_sentiment(word, method = "syuzhet"))

fire_trend_grouped <- fire_trend %>% 
  group_by(timestamp) %>% 
  summarise(count=n())
...
**Topic: Hostage Standoff**

```{r}

hostage_trend <- mb_words2 %>% 
  filter(word %in% hostage_terms_list)%>% 
  mutate(sentiment = get_sentiment(word, method = "syuzhet"))

hostage_trend_grouped <- hostage_trend %>% 
  group_by(timestamp) %>% 
  summarise(count=n())

hostage_trend_grouped %>% 
  plot_ly(x=~timestamp, y=~count) %>% 
  add_lines(colors="red")
...
rally_trend <- mb_words2 %>% 
  filter(word %in% rally_terms_list)%>%
  mutate(sentiment = get_sentiment(word, method = "syuzhet"))

rally_trend_grouped <- rally_trend %>% 
  group_by(timestamp) %>% 
  summarise(count=n())

rally_trend_grouped %>% 
  plot_ly(x=~timestamp, y=~count, colors="set2") %>% 
  add_lines()

```

From the results below, we see that there is a lot of mentions for the fire topic from 6.40pm to 6.50pm, understandable as the fire had just broken out and was being reported. 

The hostage topic reached a frenzy between 7.40pm to 7.50pm, this may indicate that something dramatic happened during this period.The hostage topics continued to have small peaks at 8.10pm, 8.20pm and 8.35pm.

We also investigated the POK rally topic, and found that it had many mentions throughout the night. 

Therefore, after analysing the results, should we be concerned that these peak periods of time poses the greatest risk to civilians? Purely looking at the frequency of topic mentions may not suffice.


### Topic Sentiment over Time

It may provide more meaning if we also find out users' sentiments when they speak about these topics throughout the night. For each topic we shall plot a chart of topic mention frequency and the total sentiment score at that particular period.

**Topic: Fire Outbreak Sentiment**

``` {r}

fire_trend_grouped_sentiment <- fire_trend %>% 
  group_by(timestamp) %>% 
  summarise(total_sentiment=sum(sentiment), count=n())

fire_trend_grouped_sentiment %>% 
  plot_ly(x=~timestamp) %>%
  add_trace(y=~count, name = 'No. of Msgs',mode = 'lines') %>% 
  add_trace(y=~total_sentiment*5, name = 'Sentiment', mode = 'lines')

```

**Topic: Hostage Standoff Sentiment**

``` {r}

hostage_trend_grouped_sentiment <- hostage_trend %>% 
  group_by(timestamp) %>% 
  summarise(total_sentiment=sum(sentiment), count=n())

hostage_trend_grouped_sentiment %>% 
  plot_ly(x=~timestamp) %>%
  add_trace(y=~count, name = 'No. of Msgs',mode = 'lines') %>% 
  add_trace(y=~total_sentiment*5, name = 'Sentiment', mode = 'lines')
...
**Topic: POK Rally Sentiment**

``` {r}

rally_trend_grouped_sentiment <- rally_trend %>% 
  group_by(timestamp) %>% 
  summarise(total_sentiment=sum(sentiment), count=n())

rally_trend_grouped_sentiment %>% 
  plot_ly(x=~timestamp) %>%
  add_trace(y=~count, name = 'No. of Msgs',mode = 'lines') %>% 
  add_trace(y=~total_sentiment*5, name = 'Sentiment', mode = 'lines')
...
Looking at the results above, it seems that for the fire and hostage topics, the sentiment score seem to be strongly inversely correlated to the topic mention frequency. It would not be too far fetched to say that the level of risks could mirror the sentiment and mention frequency - periods of large negativity could indicate people are very concerned for their safety.

The POK Rally chart unfortunately does not tell us much, as there are certain periods where the sentiment score looked to be inversely correlated to the mention frequency, yet there are also periods where higher mention counts come along with higher sentiment scores. There are also periods where the sentiment scores are 0, meaning people were mostly in a neutral state speaking about the topic.
...
From the above network results, we can roughly identify 7 distinct users who are positioned directly in he centre of the network, indicating that they are the most retweeted users and hence would most probably be a reliable source of information.

Of the 7 users, the handles for 5 of them ("KronosStar", "AbilaPost", "Central Bulletin", "InternationalNews", "NewsOnlineToday") highly suggests that they belong to a news outlet, thereby enforcing the view that these users are reporters. Another user ("HomelandIlluminations") does not have a name that sounds like a traditional news outlet, but nevertheless their microblog message style and syntax seems to be one of reporting nature. The last user is an individual with the handle "megaman". He was retweeted a lot as he was present at the scene of one of the major events and was reporting it almost in real-time as it unfolded. Together these 7 users may hold the key to unfolding the key timeline of events on the night of January 23, 2014.
...
datatable(combined_dt_new, filter = 'top', width = '200%')

```

## Event Timeline

Based on analysis of our important users' tweets, we are able to reconstruct a history of the timeline of events that happened that night. There are mainly two events that happened, a huge fire breakout at Dancing Dolphin apartment, and a hostage standoff situation at Gelato Galore. 

Let us take a look at the time visualization below.


```{r echo=TRUE}

fire_mb_rows <- c(122,154,251,354,406)
fire_cc_rows <- c(63,66,101)
hostage_mb_rows <- c(197,208,253,395)
hostage_cc_rows <- c(158,166,169,177,178,230)
...
fire_mb <- fire_mb %>% transmute(id=1:5,group=source,start=datetime,content=mb_message,title=NA)
...
fire_cc <- fire_cc %>% transmute(id=6:8,group=source,start=datetime,content=cc_message,title=cc_location)
...
hostage_mb <- hostage_mb %>% transmute(id=9:12,group=source,start=datetime,content=mb_message,title=NA)
...
hostage_cc <- hostage_cc %>% transmute(id=13:18,group=source,start=datetime,content=cc_message,title=cc_location)
...
timevis(data = combined_mbcc, groups = combined_groups, width = "1366px" ,height = "768px")
...
From the timeline of events, we can see that the fire at Dancing Dolphin apartment was a huge one. It was first reported at around 6.40pm, and 2 fire trucks were dispatched to handle the fire. However, the fire got worse, with officials deciding to evacuate neighbouring buildings just before 7pm. They also called for additional backup, although the additional fire trucks only arrived at 7.15pm. Police units were also deployed when the fire started to manage crowd control and provide assistance in terms evacuating the area. This early stages of the fire are the most risky, since there are still many residents who may not have evacuated yet, and the fire department is not yet fighting the fire at full strength. The fire continues to worsen with little that can be done to save the building, eventually the top floors of the building collapsed just after 8.30pm and an explosion was heard at 9.30pm.


The hostage standoff situation initially starts off innocuously, with the first reported event being a hit & run incident by the suspects' black van at 7.15pm. However, it could be the case that the police department did not seem to do a good job of catching the suspect, allowing the suspects to roam freely in the city for close to 10 minutes (between 7.20-7.30pm) before they were caught for a traffic violation and entered into a chase that ended up in a standoff at Gelato Galore. A backward analysis of the van's whereabouts can tell us roughly the route that the van took from the hit-and-run accident to Gelato Galore. The picture below outligns the black van's route.

![](img/37. Black Van Route.JPG){width=60%}


The van ultimately reached Gelato Galore at 7.40pm, and engaged in a dramatic standoff, where shots were fired, hostages were taken and the SWAT team was dispatched. However, kudos to the police and SWAT team, they only managed to hold out for less than 30 minutes before they were apprehended and released the hostages at around 9.10pm.


### Analysis

Retrospectively, if the police had acted quickly on the tip regarding the suspicious black van at Egeou Ave and apprehended the criminals there and then, maybe the entire hostage standoff and shootout could have been avoided.

However, things never happen in retrospect. It is also understandable that the police had their hands full as well with the fire at Dancing Dolphin. However, we can see a window where the police might have acted differently. The additional fire department personnel arrived at 7.15pm, slightly before the hit & run accident happened. With additional resources on the ground, the police could have possible devoted more units to search for the hit and run suspects. However, they did not do this, instead sending yet another police resource to the fire site.

![](img/36. Additional Police at 716.JPG){width=60%}
