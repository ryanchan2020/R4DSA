...
datatable(RT_msg_DT, filter = 'top')

```

Based on analysis of our important users' tweets, below is a history of the timeline of events that happened that night:
...
cc_dt <- cc %>%
  ...
  transmute(call_type=case_type, message=message, time=timestamp,location=location) %>% 
  arrange(!(desc(time)))
